...
public static class SavWav
{
    ...
    public static void SaveRaw(string filePath, byte[] bytes, uint length)
    {
        if (!filePath.EndsWith(".wav", StringComparison.OrdinalIgnoreCase))
        {
            filePath += ".wav";
    ...
    public static byte[] GetWav(AudioClip clip, float[] samples)
    {
        ...
        WriteHeader(buffer, clip, p, (uint)(end - start + 1));

...
namespace CrazyMinnow.SALSA.OneClicks
{
	public class OneClickCC3Eyes : MonoBehaviour
	{
		public static void Setup(GameObject go)
		{
			...
			string[] eyeL = new string[] {"l_eye$", "leye", "Eye_L", "Eyes_L"};
			string[] eyeR = new string[] {"r_eye$", "reye", "Eye_R", "Eyes_R"};
			...
			if (go)
			{
                ...
                Transform eyeLeft = Eyes.FindTransform(eyes.characterRoot, eyeL);
                Transform eyeRight = Eyes.FindTransform(eyes.characterRoot, eyeR);
                if (eyeLeft != null && eyeRight != null)
                {
	                if (eyeLeft.GetComponent<SkinnedMeshRenderer>())
	                {
		                eyes.eyes[0].expData.components[0].controlType = ExpressionComponent.ControlType.Shape;
		                eyes.eyes[0].expData.controllerVars[0].smr = eyeLeft.GetComponent<SkinnedMeshRenderer>();
	                }
	                eyes.eyes[0].expData.controllerVars[0].bone = eyeLeft;
	                eyes.eyes[0].expData.name = "eyeL";
	                eyes.eyes[0].expData.components[0].name = "eyeL";
	                
	                if (eyeRight.GetComponent<SkinnedMeshRenderer>())
	                {
		                eyes.eyes[1].expData.components[0].controlType = ExpressionComponent.ControlType.Shape;
		                eyes.eyes[1].expData.controllerVars[0].smr = eyeRight.GetComponent<SkinnedMeshRenderer>();
	                }
	                eyes.eyes[1].expData.controllerVars[0].bone = eyeRight;
	                eyes.eyes[1].expData.name = "eyeR";
	                eyes.eyes[1].expData.components[0].name = "eyeR";
                }
                else
                {
	                Transform bothEyes = Eyes.FindTransform(eyes.characterRoot, "Eye");
	                if (bothEyes.GetComponent<SkinnedMeshRenderer>())
	                {
		                eyes.eyes[0].expData.components[0].controlType = ExpressionComponent.ControlType.Shape;
		                eyes.eyes[0].expData.controllerVars[0].smr = bothEyes.GetComponent<SkinnedMeshRenderer>();
	                }
	                eyes.eyes[0].expData.controllerVars[0].bone = bothEyes;
	                eyes.eyes[0].expData.name = "eyes";
	                eyes.eyes[0].expData.components[0].name = "eyes";
	                
	                eyes.eyes.RemoveAt(1);
                }

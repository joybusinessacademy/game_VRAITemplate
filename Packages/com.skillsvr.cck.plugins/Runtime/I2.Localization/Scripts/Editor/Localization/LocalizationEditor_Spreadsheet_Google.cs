...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void OnGUI_Spreadsheet_Google()
		{
			...
			if (isInRestrictedMode(target as ILanguageSource))
			{
				GUILayout.BeginVertical();
				EditorGUILayout.LabelField("Sheet Name: " + mProp_Google_SpreadsheetName.stringValue);
				EditorGUILayout.LabelField("Key: " + mProp_Google_SpreadsheetKey.stringValue);
				if (GUILayout.Button("Import & Replace translations"))
				{
					Import_Google(eSpreadsheetUpdateMode.Replace);
				}
		...
		bool isInRestrictedMode(ILanguageSource target) => (target as ILanguageSource).SourceData.mIsGlobalSource == false;
		...
		void OnGUI_GoogleCredentials()
		{
			...
			if (isInRestrictedMode(target as ILanguageSource))
			{
				GUILayout.Label(new GUIContent("Web Service URL: " + mProp_Google_WebServiceURL.stringValue, WebServiceHelp));
			}
			else
			{
				GUILayout.BeginHorizontal();
		...
		void OnImported_Google( string Result, string Error, eSpreadsheetUpdateMode UpdateMode )
		{
			...
			// Removes empty text
			source.mTerms.RemoveAll(key => key.Term.LastIndexOf('/') == key.Term.Length - 1);

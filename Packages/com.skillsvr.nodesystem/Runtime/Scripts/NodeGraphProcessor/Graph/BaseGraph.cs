...
namespace GraphProcessor
{
	...
	public abstract class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		/// <returns></returns>
        [SerializeReference, FormerlySerializedAs("commentBlocks")]
		...
		/// <summary>
		/// The graph instance id.
		/// Instance copies of a graph asset will have different UUID but have same asset path.
		/// </summary>
		...
		public Vector3 position
		{
			get
			{
				...
				float x = EditorPrefs.GetFloat(path + "-graph-X-Pos", 0);
				float y = EditorPrefs.GetFloat(path + "-graph-Y-Pos", 0);
				float z = EditorPrefs.GetFloat(path + "-graph-Z-Pos", 0);
...
#if  UNITY_EDITOR
				string path = AssetDatabase.GetAssetPath(this);
				EditorPrefs.SetFloat(path + "-graph-X-Pos", value.x);
				EditorPrefs.SetFloat(path + "-graph-Y-Pos", value.y);
				EditorPrefs.SetFloat(path + "-graph-Z-Pos", value.z);
#endif
			}
		}
		...
		public Vector3 scale
		{
			get
			{
				...
				float x = EditorPrefs.GetFloat(path + "-graph-X-Scl", 1);
				float y = EditorPrefs.GetFloat(path + "-graph-Y-Scl", 1);
				float z = EditorPrefs.GetFloat(path + "-graph-Z-Scl", 1);
				return new Vector3(x, y, z);
#else
				return Vector3.one;
#endif
			...
			set
			{
				...
				EditorPrefs.SetFloat(path + "-graph-X-Scl", value.x);
				EditorPrefs.SetFloat(path + "-graph-Y-Scl", value.y);
				EditorPrefs.SetFloat(path + "-graph-Z-Scl", value.z);
#endif
		...
		/// <typeparam name="BaseNode"></typeparam>
		/// <returns></returns>
		public virtual List<BaseNode> Nodes
		{
			get => nodes;
			set => nodes = value;
		}
		...
		public BaseGraph()
		{
			...
			//position = new Vector3(80f, 175f, 0f);
		}

        public override string ToString()
        {
			return string.Join(" ", this.name, this.UUID, this.GetInstanceID(), this.GetHashCode());
        ...
        public virtual void OnEnable()
        {
			...
			if (0 == nodes.Count())
			{
				AddNode(BaseNode.CreateFromType(typeof(StartNode), Vector2.zero));
				AddNode(BaseNode.CreateFromType(typeof(EndNode), Vector2.right * 400));
			}
        }


		public virtual void BuildGraphElements(bool forceRebuild  = false)
		{
			if (isEnabled && !forceRebuild)
			{
				return;
			}
		...
		/// </summary>
		/// <param name="block"></param>
		public void AddGroup(LogicGroup block)
		{
			groups.Add(block);
			onGraphChanges?.Invoke(new GraphChanges { addedGroups = block });
		}
		...
		{
			if (block is LogicGroup logicGroup)
			{
				var node = logicGroup.GetEnterGroupNode(this);
				nodes.Remove(node);
				onGraphChanges?.Invoke(new GraphChanges{ removedNode = node });
				
				var node1 = logicGroup.GetExitGroupNode(this);
				nodes.Remove(node1);
				onGraphChanges?.Invoke(new GraphChanges{ removedNode = node1 });
			}
...
#pragma warning restore CS0618
		...
		{
			Nodes.OfType<ISaveHandler>().ForEach(x => x.OnBeforeSave());
#if UNITY_EDITOR
			if (this != null)
			{

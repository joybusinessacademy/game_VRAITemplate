...
namespace GraphProcessor
{
	...
	public delegate void CustomNodeEventDelegate(BaseNode node, object eventKey, object data);

	[Serializable]
	public abstract class BaseNode
	{
		// prevent null call
		public static CustomNodeEventDelegate customNodeEvent = delegate{};
		...
		/// </summary>
		public virtual Color color => NodeColours.Other;
		
		/// <summary>
		/// The width of the node
		/// </summary>
		public virtual int Width => SMALL_WIDTH;
		
		public const int SMALL_WIDTH = 140;
		public const int MEDIUM_WIDTH = 240;
		
		/// <summary>
		/// If the whole node is one color
		/// </summary>
		public virtual bool SolidColor => false;
		...
		public virtual string tooltipURL => "";
        ...
        /// <summary>
        /// The node guid.
        /// Nodes will share same GUID but have different UUID if they are in different instances of a graph asset.
        /// </summary>
        public string				GUID;

        /// <summary>
        /// The runtime node instance guid. 
        /// Nodes will share same GUID but have different UUID if they are in different instances of a graph asset.
        /// </summary>
		...
		public event Action<NodePort> onNodePortActived;
		...
		void InitializeInOutDatas()
		{
			...
			foreach (FieldInfo field in fields)
			{
				...
				OutputAttribute outputAttribute = field.GetCustomAttribute< OutputAttribute >();
				...
				VerticalAttribute vertical = field.GetCustomAttribute< VerticalAttribute >();
			...
			{
				CustomPortBehaviorAttribute customPortBehaviorAttribute = method.GetCustomAttribute< CustomPortBehaviorAttribute >();
        ...
        {
			return string.Join(" ", this.name, this.GUID, this.UUID);
        }

		public void SendCustomEvent(object key, object value)
		{
			customNodeEvent?.Invoke(this, key, value);
        }

		public NodePort GetInputPortByName(string name)
		{
			return inputPorts.FirstOrDefault(x => null != x && name == x.fieldName);
        }

        public NodePort GetOutputPortByName(string name)
        {
            return outputPorts.FirstOrDefault(x => null != x && name == x.fieldName);
        }

		public void TriggerInputPortActiveByName(string name)
		{
            ...
            onNodePortActived?.Invoke(port);
		}

        public void TriggerOutputPortActiveByName(string name)
        {
            var port = GetOutputPortByName(name);
            if (null == port)
            {
                return;
            }
        ...
        public void TriggerPortActiveByName(string name)
        {
            var port = GetPortByName(name);
            if (null == port)
            {
                return;
            }
        ...
        public NodePort GetPortByName(string name)
		{
			var port = GetOutputPortByName(name);
			if (null != port)
			{
			...
			return GetInputPortByName(name);
		}

        public IEnumerable<SerializableEdge> GetAllEdgesByOutputName(string outputFieldName)
		{
			if (string.IsNullOrWhiteSpace(outputFieldName))
			{
				return new List<SerializableEdge>(0);
			}

			var values = this.outputPorts
				?.FirstOrDefault(p => null != p && outputFieldName == p.fieldName)
				?.GetEdges();
			if (null == values)
			{
				return new List<SerializableEdge>(0);
		...
		public BaseNode GetFirstNodeByInputName(string inputFieldName)
		{
			return GetAllEdgesByInputName(inputFieldName)
					?.FirstOrDefault(e => null != e && null != e.outputNode)
					?.outputNode;
		}

		public BaseNode GetFirstNodeByOutputName(string outputFieldName)
		{
			return GetAllEdgesByOutputName(outputFieldName)
					?.FirstOrDefault(e => null != e && null != e.outputNode)
					?.inputNode;
		}

		public IEnumerable<SerializableEdge> GetAllEdgesByInputName(string inputFieldName)
		{
			...
			var values = this.inputPorts
				?.FirstOrDefault(p => null != p && inputFieldName == p.fieldName)
				?.GetEdges();
			if (null == values)
			{
				return new List<SerializableEdge>(0);
		...
		public virtual object RetrieveDataFromInputName(string inputFieldName)
		{
			if (string.IsNullOrWhiteSpace(inputFieldName))
			{
				return null;
			}
			var edge = GetAllEdgesByInputName(inputFieldName)
				.FirstOrDefault(e => null != e && null != e.outputNode);
			return RetrieveDataByEdge(edge);
		}

		protected virtual object OnReceiveDataRetrieveFromOutputName(string outputFieldName)
		{
			return null;
		}

		public virtual object RetrieveDataByEdge(SerializableEdge inputEdge)
		{
			if (null == inputEdge)
			{
				return null;
			}
			var prevDataSourceNode = inputEdge.outputNode;
			if (null == prevDataSourceNode)
			{
				return null;
			}

			return prevDataSourceNode.OnReceiveDataRetrieveFromEdge(inputEdge);
		}

		protected virtual object OnReceiveDataRetrieveFromEdge(SerializableEdge outputEdge)
		{
			if (null == outputEdge)
			{
				return null;
			}
			return OnReceiveDataRetrieveFromOutputName(outputEdge.outputFieldName);
		}
	}
}

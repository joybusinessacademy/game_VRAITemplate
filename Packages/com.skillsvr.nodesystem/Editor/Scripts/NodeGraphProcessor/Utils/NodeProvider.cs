...
namespace GraphProcessor
{
	public static class NodeProvider
	{
		...
		public static void HookForBuildCacheForNode(Type nodeType, BaseGraph graph = null)
        {
			BuildCacheForNode(nodeType, genericNodes, graph);
        }
		...
		private static Dictionary<Type, IEnumerable<FieldInfo>> managedNodeTypeFields = new Dictionary<Type, IEnumerable<FieldInfo>>();

		public static IEnumerable<FieldInfo> GetAllFieldInfoFromType(BaseNode baseNode)
		{
			Type type = baseNode.GetType();
			IEnumerable<FieldInfo> output = null;
			if (null == type)
			{
				return output;
			}

			if (managedNodeTypeFields.TryGetValue(type, out output))
			{
				return output;
			}

			output = type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
			...
			output = baseNode.OverrideFieldOrder(output).Reverse();

			managedNodeTypeFields.Add(type, output);

...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class PlaylistMediaPlayer : MediaPlayer, IMediaProducer
	{
		...
		private void SetTransition(Transition transition, float duration, Easing.Preset easing)
		{
			...
			{
				transition = (Transition)UnityEngine.Random.Range(0, (int)Transition.Random);
			}
			...
			public static System.Func<float, float> GetFunction(Preset preset)
			{
				...
				switch (preset)
				{
					...
					case Preset.InOutExpo:
						...
						break;
					case Preset.Random:
						result = GetFunction((Preset)UnityEngine.Random.Range(0, (int)Preset.Random));
						break;
					case Preset.RandomNotStep:
						result = GetFunction((Preset)UnityEngine.Random.Range((int)Preset.Step+1, (int)Preset.Random));
						break;
				}
				return result;
			}

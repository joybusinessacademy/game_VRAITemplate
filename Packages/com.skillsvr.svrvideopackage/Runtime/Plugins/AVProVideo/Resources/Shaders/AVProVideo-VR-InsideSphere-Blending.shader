Shader "AVProVideo/VR/InsideSphere Unlit (stereo+fog) with Blending"
{
	Properties
	{
		...
		_AlternateTexWeight("Alternate Texture Weight", Range(0.0, 1.0)) = 0.0
		...
		float3 RotateAroundYInDegrees(float3 vertex, float degrees)
		{
			...
			return float3(mul(m, vertex.xz), vertex.y).xzy;
				...
				uniform sampler2D _AlternateTex;
				...
				v2f vert(appdata v)
				{
					...
					if (_AlternateTexWeight > 0.0)
					{
						if (!IsStereoEyeLeft(_cameraPosition, UNITY_MATRIX_V[0].xyz))
						{
							o.uv.xy = TRANSFORM_TEX(v.uv2, _AlternateTex);
							o.uv.xy = float2(1.0 - o.uv.x, o.uv.y);
							o.uv.xy *= _AlternateTex_ST.xy;
							o.uv.xy += _AlternateTex_ST.zw;
						}
					}
					else
					{
				...
				else
				{
...
#endif
#else
					uv = TRANSFORM_TEX(i.uv, _AlternateTex);
#endif
				}
					fixed4 col;
	#if USE_YPCBCR
					...
					if (_AlternateTexWeight > 0.0)
					{
						col = lerp(SampleRGBA(_MainTex, uv), SampleRGBA(_AlternateTex, uv), _AlternateTexWeight);
					}
					else
					{

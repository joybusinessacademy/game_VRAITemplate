...
Shader "Skybox/Panoramic Swap" {
Properties {
    _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
    [Gamma] _Exposure ("Exposure", Range(0, 8)) = 1.0
    _Rotation ("Rotation", Range(0, 360)) = 0
    [NoScaleOffset] _MainTex ("Spherical  (HDR)", 2D) = "grey" {}
	[NoScaleOffset] _AlternateTex ("Alternate Spherical  (HDR)", 2D) = "grey" {}
	_Weight ("Alternate Weight", Range(0,1)) = 0
...
SubShader {
    ...
    Pass {
        ...
        fixed4 frag (v2f i) : SV_Target
        {
			...
			half4 tex2 = tex2D (_AlternateTex, tc);
			tex = lerp(tex,tex2, _Weight);

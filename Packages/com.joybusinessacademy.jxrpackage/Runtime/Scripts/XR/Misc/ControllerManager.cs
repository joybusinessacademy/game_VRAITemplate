...
namespace JBA.XRPlayerPackage
{
    ...
    public class ControllerManager : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("The buttons on the controller that will trigger a transition to the Teleport Controller.")]
        List<InputHelpers.Button> m_SecondaryButton = new List<InputHelpers.Button>();
        ...
        [SerializeField]
        [Tooltip("The Game Object which represents the pointing hand")]
        GameObject m_LeftPointingController;
        ...
        [SerializeField]
        [Tooltip("The Game Object which represents the pointing hand")]
        GameObject m_RightPointingController;
        /// <summary>
        ...
        public XRRayInteractor eyeRayInteractor;
        [FormerlySerializedAs("jxrLeftHand")] [SerializeField] public SVRHands svrLeftHand;
        [FormerlySerializedAs("jxrRightHand")] [SerializeField] public SVRHands svrRightHand;
        ...
        {
            public ControllerStates currentState;
            public ControllerStates defaultState;
            ...
            public void SetState(ControllerStates nextState)
            {
                ...
                else
                {
                    ...
                    {
                        m_Interactors[(int)currentState].Leave();
                    }

                    currentState = nextState;
                    m_Interactors[(int)currentState].Enter();
                }
            }
        }

        public ControllerState m_RightControllerState;
        public ControllerState m_LeftControllerState;

        public void SetController(int index, ControllerStates state)
        {
            // use left hand
            if (index == 0)
            {
                m_LeftControllerState.defaultState = state;
                m_LeftControllerState.SetState(state);
            }
            else
            {
                m_RightControllerState.defaultState = state;
                m_RightControllerState.SetState(state);
            }
        }

        public void SetLeftController(ControllerStates state)
        {
            m_LeftControllerState.defaultState = state;
            m_LeftControllerState.SetState(state);
        }

        public void SetRightController(ControllerStates state)
        {
            m_RightControllerState.defaultState = state;
            m_RightControllerState.SetState(state);
        }
        ...
        void OnEnable()
        {
            ...
            m_RightControllerState.SetGameObject(ControllerStates.Holding, m_RightBaseController);
            m_RightControllerState.SetGameObject(ControllerStates.Point, m_RightPointingController);
            ...
            m_LeftControllerState.SetGameObject(ControllerStates.Holding, m_LeftBaseController);
            m_LeftControllerState.SetGameObject(ControllerStates.Point, m_LeftPointingController);
        ...
        void Update()
        {
            if (m_LeftController.isValid)
            {
                ...
                {
                    m_LeftControllerState.SetState(m_LeftControllerState.defaultState);

                    if (!activated)
                        m_LeftTeleportDeactivated = false;
                }

                //m_LeftController.IsPressed(m_SecondaryButton[0], out bool jvalue);
                svrLeftHand.UpdateButtons(m_LeftController);
            ...
            if (m_RightController.isValid)
            {
                ...
                {
                    m_RightControllerState.SetState(m_RightControllerState.defaultState);

                    if (!activated)
                        m_RightTeleportDeactivated = false;
                }

                svrRightHand.UpdateButtons(m_RightController);
            }

            // not good
            // why do i have this here
            if (previousRightHand != m_RightControllerState.currentState)
            {
                if (m_RightControllerState.currentState == ControllerStates.Point)
                    svrRightHand.selfAnimator.SetPose(HandsAnimatorControl.HandPose.Point);
                else if (m_RightControllerState.currentState == ControllerStates.Select)
                    svrRightHand.selfAnimator.SetPose(HandsAnimatorControl.HandPose.Fist);
            }
            previousRightHand = m_RightControllerState.currentState;

            if (previouLeftHand != m_LeftControllerState.currentState)
            {
                if (m_LeftControllerState.currentState == ControllerStates.Point)
                    svrLeftHand.selfAnimator.SetPose(HandsAnimatorControl.HandPose.Point);
                else if (m_LeftControllerState.currentState == ControllerStates.Select)
                    svrLeftHand.selfAnimator.SetPose(HandsAnimatorControl.HandPose.Fist);
            }

            previouLeftHand = m_LeftControllerState.currentState;
        }

        private ControllerStates previousRightHand;
        private ControllerStates previouLeftHand;
        public bool AnyControllerPressedButton(InputHelpers.Button targetButton)
        {
            ...
            if (m_LeftController.isValid)
                m_LeftController.IsPressed(targetButton, out leftvalue);
            if (m_RightController.isValid)
                m_RightController.IsPressed(targetButton, out rightvalue);

            return leftvalue || rightvalue;
        }

        public bool IsButtonPressed(bool left, InputHelpers.Button targetButton)
        {
            ...
            {
                if (m_LeftController.isValid)
                    m_LeftController.IsPressed(targetButton, out ret);
            }

            if (left == false)
            {
                if (m_RightController.isValid)
                    m_RightController.IsPressed(targetButton, out ret);
            }
